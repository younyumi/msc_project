...
class PathTrackingNode:
    ...
    def find_nearest_point(self):
        ...
        for i, (x_ref, y_ref, z_ref) in enumerate(zip(self.x_ref, self.y_ref, self.z_ref)):
            distance = ((self.cur_x - x_ref) ** 2 +
                        (self.cur_y - y_ref) ** 2 +
                        (self.cur_z - z_ref) ** 2) ** 0.5
            if distance < self.min_distance:
                self.min_distance = distance
                self.Index = i
        next_index = (self.Index + 1) % len(self.x_ref)

        self.nearest_Path = [self.x_ref[next_index], self.y_ref[next_index], self.z_ref[next_index]]
    ...
    def kanayama_control(self):
        # Kanayama 제어기 로직
        x_e = (self.nearest_Path[0] - self.cur_x) * math.cos(self.cur_theta) + \
              (self.nearest_Path[1] - self.cur_y) * math.sin(self.cur_theta)
        y_e = -(self.nearest_Path[0] - self.cur_x) * math.sin(self.cur_theta) + \
              (self.nearest_Path[1] - self.cur_y) * math.cos(self.cur_theta)

        theta_e = (theta_e + math.pi) % (2 * math.pi) - math.pi

        v_ref = 20 / 3.6  # 20 km/h
        w_ref = 0.05
        kx = 4
        ky = 0.3
        kw = 0.6
        ...
        throttle, brake = self.dynamic_speed_control(v_ref)
